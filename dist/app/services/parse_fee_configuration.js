module.exports = (feeConfig) => {
    ...
    return filteredByLineBreak.map(configLine => {
        ...
        let feeId = configElementsArray[0], currency = configElementsArray[1], locale = configElementsArray[2], entity = configElementsArray[3].split("(").filter(config => config)[0], entityProperty = configElementsArray[3].match(/\(([^)]+)\)/)[1], type = configElementsArray[6], value = {};
        if (type === 'PERC') {
